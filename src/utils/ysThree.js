...
(function (W,doc,U) {
  ...
  const createSpriteText = ( text, options ) => {
    ...
    sprite.scale.set(options.fontSiGeoze / average * text.length, options.fontSize / average, 1)
    return sprite
  }
  ...
  const getMeshArea = mesh => {
    ...
    if( geometry.isBufferGeometry) geometry =  new THREE.BufferGeometry().fromBufferGeometry(geometry);
