<template>
  ...
  <div class="space  " id="user-home">
    ...
    <div class="section">
      ...
      <div class="section-content user-home-header  user-home-section">
        <a-row
          ...
          <a-col :span="18" class="col">
            <a-row class="user-info" type="flex" justify="start" :gutter="32">
              <a-col :span="4" class="avatar">
                ...
                <!--                  </a-button>-->
                <!--                  &lt;!&ndash;                <span class="edit-btn" @click="openUserEditForm">{{ hasUplink ? '编辑信息' : '生成虚拟身份' }}</span>&ndash;&gt;-->
                <!--                </a-space>-->
              </a-col>
...
<!--                                </div>-->
    ...
    <a-row style="width: 1400px" type="flex" class="user-home-section" justify="space-between">
      <a-col :span="14">
      ...
      <a-col :span="10">
        <div class="glass-morphism app-card" style="height: 400px;float: right">
          <span>
            成长维度
          </span>
...
import { EditUserInfo, GetUserExternalInfo, UserToBlockChain } from '@/api/modules/user'
...
export default {
  ...
  async mounted () {
    // 进入此页面拿去用户的信息
    this.userInfo = await this.getLatestUserInfo()
    this.hasUplink = await this.$store.getters.hasUpLink
    ...
    const info = await GetUserExternalInfo({ userId })
