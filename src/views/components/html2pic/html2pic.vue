<template>
  <div class="box">
    <a-modal
      ...
      <div
        :id="`id-${id}`"
        class="flex-col-space-between pic"
        ref="pic"
        ...
        <div class="content" :style="option.fontSize?`font-size:${option.fontSize}`:''">{{ info.eviContent }}</div>
        <div class="bottom">
          <div class="signs flex-col-space-around-reverse">
            <div class="sign" v-for="s in info.sign" :style="option.signFontSize?`font-size:${option.signFontSize}`:''">
...
import { CertificateDetail } from '@/api/modules/certificate'
...
export default {
  ...
  props: {
    ...
    // src: {
    //   type: String,
    //   default: require('@/assets/certificate/z_1.png')
    // }
  },
  data () {
    return {
      ...
      getType: certificateConfig.getPicType,
      logo: certificateConfig.L_logo
    }
  },
  methods: {
    init () {
      const name = this.getType(this.info.certificateType)
      this.option = certificateConfig[name]
      console.log(this.option)
    },
    getDetail () {
      // this.loading = truewith
      this.init()
      CertificateDetail({ id: this.id }).then(res => {
        if (res.ok) {
          this.info = res.data
          this.loading = false
          this.init()
    ...
    generateImage: function () {
      ...
      const canvasID = this.$refs['pic']
      const _this = this
      const a = document.createElement('a')
      // 获取元素的可滚动内容的高度
      const height = document.querySelector(`#id-${this.id}`).scrollHeight
      ...
      html2canvas(canvasID).then(function (canvas) {
        ...
        const blob = _this.dataURLToBlob(dom.toDataURL('image/png'))
