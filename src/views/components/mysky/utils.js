...
export function DrawSky(dom, innerHeight) {
  // const innerWidth=window.innerWidth
  // 清除控制台
  console.clear()
  const ballNumber = 5000
  //color
  // const color1='(125.,117.,0.)'
  // const color2='(125.,117.,255.)'
  const color1 = '(125.,117.,0.)'
  const color2 = '(4, 116, 188)'
// 创建场景
  let scene = new THREE.Scene()
// 设置场景背景颜色
  scene.background = new THREE.Color(0x14232d)
...
// 创建一个长度为5万的数组pts并y用Array.prototype.map()方法遍历数组并对每个元素进行操作
  let pts = new Array(ballNumber).fill().map(p => {
    ...
    onBeforeCompile: shader => {
      ...
      shader.vertexShader = `
        ...
        .replace(
          ...
          `#include <color_vertex>
                    float d = length(abs(position)/vec3(0.,0.,0));
                    d=clamp(d,0.,1.);
                    vColor = mix(vec3${color1},vec3${color2},d)/255.;`
        )
        // 记得加上分号
        // 更新点的移动
        .replace(
